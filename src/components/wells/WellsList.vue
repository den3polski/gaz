  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Скважины</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
  ...
  export default {
    ...
    data: () => ({
      ...
      headers: [
            ...
            {text: 'Глубина проектная', value: 'depthProject' },
            {text: 'Принцип использования открытого гнунта', value: 'useGroundPrinciple' },
            {text: 'СМС', value: 'sms' },
            {text: 'Температура проектная', value: 'temperatureProject' },
    ...
    watch: {
      ...
      filter: {
          handler() {
                this.items = ipcRenderer.sendSync('getEntities', {typeOfEntity: 'Well', filter: this.filter})     
    ...
    methods: {
      ...
      prepareWell() {
          console.log(1)
         if (this.filter.cluster !== null) {
             this.editedItem.cluster = {...this.filter.cluster}
         }        
      },
